...
class DecoderLayer(nn.Module):
    ...
    def forward(self, input, memory, reference_points, spatial_shapes, query_embed: Tensor, memory_key_padding_mask: Optional[Tensor] = None):
        """
        - input: [B, num_queries, embed_dim]
        ...
        # [B, num_queries, C]
        k_queries = q_queries = self.with_pos_embed(input, query_embed)
        v_queries = input
        ...
        q_memory = self.with_pos_embed(add_norm_out, query_embed)
        # resize the reference_points to the right size: [batch, query_len, num_levels, RpQ, 2]
        # [query_len, RpQ, 2] -> [batch, query_len, num_levels, RpQ, 2]
        reference_points = reference_points[None, :, None, :, :].expand(B, Q, self.num_levels, self.num_ref_points_per_query, 2).contiguous()
        ...
        # memory_attention_weights: [batch, query_len, heads, num_levels, num_points, RpQ]
        # result: [B, Q, embed_dim]
        memory_attention_out = self.dropout2(memory_attention_out)
        # add and normalize
        # add_norm_out: [B, num_queries, embed_dim]
        # memory_attention_out: [B, Q, embed_dim]
        # [B, num_queries, embed_dim]
