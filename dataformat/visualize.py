...
def visualize_one(annotation: Annotation) -> Image.Image:
    ...
    """
    image = Image.open(Path("out/images") / annotation.image.url)
    w, h = image.size
...
if __name__ == "__main__":
    import json
    with open("out/annotations/cloth_7_buttons_fastened_distractions_00000000.json") as file:
        json_data = json.load(file)
        ann = Annotation.from_json(json_data)
        image = visualize_one(ann)
        image.save("out.png")